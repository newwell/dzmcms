...
<table width="97%" cellpadding="1" cellspacing="1" align="center" class="listtable">
        ...
        <tr bgcolor="#F1F3F5" onmouseover="tr_add_color($(this))"onmouseout="tr_del_color($(this))">
			...
			<?php if ($value['status']=="已入赛"){?>
				...
				<?php if ($value['sport']['type']=="time_trial") {?>
					<a onclick="if(confirm('确定现在退赛,停止服务费的计算!')){location.href='?action=sport_withdraw&todo=dowithdraw&entry_id=<?php echo $value['id']?>&card=<?php echo $value['card'];?>&sport_id=<?php echo $value['sport']['id'];?>'}" href="JavaScript:;" title="退赛">退赛</a>
				<?php }elseif ($value['sport']['type']=="no_time_trial"){?>
					<a onclick="if(confirm('确定将积分退回?')){location.href='?action=sport_withdraw&todo=dowithdraw&entry_id=<?php echo $value['id']?>&card=<?php echo $value['card'];?>&sport_id=<?php echo $value['sport']['id'];?>'}" href="JavaScript:;" title="退赛">退赛</a>
				<?php }elseif ($value['sport']['type']=="pk_trial"){?>
					<a onclick="if(confirm('确定颁奖?')){location.href='?action=sport_withdraw&todo=dowithdraw&entry_id=<?php echo $value['id']?>&card=<?php echo $value['card'];?>&sport_id=<?php echo $value['sport']['id'];?>'}" href="JavaScript:;" title="颁奖">颁奖</a>
				<?php }?>
